...
\item Zbiór drzew DLS $\mathcal{T}_i=\{T_i^1,T_i^2, \dots , T_i^h\}$ dla każdego drzewa genów $G_i \in \mathcal{G}$, reprezentowanych jako wektory  epizodów $\langle e^1_{i_j},e^2_{i_j}, \dots , e^m_{i_j} \rangle$, gdzie dla każdego $k$, $e^k_{i_j}$ to liczba epizodów duplikacyjnych przypisanych do węzła $s_k$ w $j$-tym drzewie DLS dla $i$-tego drzewa genów.
\end{enumerate}}
